...
def get_columns_list_with_cursor(cur, table_schema, table_name):
    cur.execute("""
    select column_name
    from information_schema.columns
    where table_schema = '{}'
...
"""
get list of columns of passed in query
"""
...
def get_query_schema(query):
    result = ""
    query = query.replace("select", "")
    split_query = query.split(" ")
    for sq in range(len(split_query)):
        if split_query[sq].strip().lower() == "from":
            result = split_query[sq+1].strip().lower()
            break
    return result.split(".")[0]


"""
get list of tables in passed in query
"""
